...
def main():
    ...
    spikes = spikes.sort_values(["disputes_within_14d", "day_txns"], ascending=False)
    spikes.to_csv(RISK / "merchant_transaction_spikes.csv", index=False)

    # ------------------------------------------------------------------ velocity (same user, many payments fast)
    ...
    mr = mr.reset_index().sort_values(["risk_score", "disputed_amount"], ascending=False)
    ...
    ur = ur.reset_index().sort_values(["risk_score", "disputed_amount"], ascending=False)
